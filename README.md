...
To elaborate further, only the Fourth Industrial Revolution employment index significantly influences the Course Impact Index (β<sub>i4.0</sub>  = 0.2024***). This implies that within Singapore's vocational education space, courses aligned with the Fourth Industrial Revolution exert a substantial impact, emphasizing the need for policy attention. Last but not the least, the regression coefficients for both job diversity and skill diversity are both negative (β<sub>job</sub> = -0.0792**, β<sub>skill</sub> = -0.3572***), suggesting that courses must be tailored to specific jobs or skill sets to maximize their impact.
