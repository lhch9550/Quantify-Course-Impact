...
To elaborate further, only the Fourth Industrial Revolution employment index significantly influences the Course Impact Index (β~i4.0 = 0.2024***). This implies that within Singapore's vocational education space, courses aligned with the Fourth Industrial Revolution exert a substantial impact, emphasizing the need for policy attention. In contrast, the regression coefficients for both job diversity and skill diversity are both negative (β~job = -0.0792**, β~skill = -0.3572***), suggesting that courses must be tailored to specific jobs or skill sets to maximize their impact.
